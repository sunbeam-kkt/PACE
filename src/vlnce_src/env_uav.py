...
class AirVLNENV:
    def __init__(self, batch_size=8, 
        ...
        self.capture_record_images = True
        self.init_VectorEnvUtil()

    def set_record_image_capture(self, enabled: bool):
        self.capture_record_images = bool(enabled)
    ...
    def _getStates(self):
        ...
        if self.capture_record_images:
            responses_for_record = self.simulator_tool.getImageResponsesForRecord()
        else:
            responses_for_record = [[([], []) for _ in machine_responses] for machine_responses in responses]
        cnt = 0
