...
def _runtime_args():
    try:
        from src.common.param import args as parsed_args
        return parsed_args
    except Exception:
        return None
...
def load_model(args):
    ...
    tokenizer, model, image_processor, _ = load_pretrained_model(
        ...
        hf_offline=bool(getattr(runtime_args, 'hf_offline', False)),
    )
    if hasattr(model, 'config'):
        model.config.bert_model_path = getattr(args, 'bert_model_path', None)
        model.config.hf_offline = bool(getattr(runtime_args, 'hf_offline', False))
        for name in [
            'use_internalized_assistant',
            'use_counterfactual_grounding',
            'use_affordance_field',
            'help_loss_scale',
            'ranking_loss_scale',
            'rejection_loss_scale',
            'affordance_loss_scale',
        ]:
            if runtime_args is not None and hasattr(runtime_args, name):
                setattr(model.config, name, getattr(runtime_args, name))
                if hasattr(model, name):
                    setattr(model, name, getattr(runtime_args, name))
    ...
    if lora_enable:
        ...
        if getattr(runtime_args, 'merge_lora_for_inference', False):
            try:
                print("Merging LoRA weights for faster inference")
                model = model.merge_and_unload()
            except Exception as exc:
                print(f"LoRA merge skipped: {exc}")
    
    if runtime_args is not None and hasattr(model, 'config'):
        model.config.empty_cache_each_forward = bool(getattr(runtime_args, 'empty_cache_each_forward', False))
...
def prepare_data_to_inputs(episodes, tokenizer, image_processor, data_args, target_point, assist_notice = None, disable_external_assistant=False):
    ...
    if disable_external_assistant:
        stage = 'none'
    elif assist_notice is not None:
        stage = assist_notice
    else:
