...
def set_record_image_capture_if_supported(eval_env: AirVLNENV, enabled: bool):
    setter = getattr(eval_env, "set_record_image_capture", None)
    if callable(setter):
        setter(enabled)
    ...
    with torch.inference_mode():
        ...
        while True:
            ...
            if env_batchs is None:
                break
            set_record_image_capture_if_supported(eval_env, False)
            ...
            for t in range(int(args.maxWaypoints) + 1):
                if args.log_step_interval <= 1 or t % args.log_step_interval == 0:
                ...
                model_wrapper.maybe_write_innovation_trace(t, batch_state, refined_waypoints)
                eval_env.makeActions(refined_waypoints)
                should_predict_done = model_wrapper.should_predict_done(t)
                set_record_image_capture_if_supported(eval_env, should_predict_done)
                outputs = eval_env.get_obs()
                batch_state.update_from_env_output(outputs)
                
                if should_predict_done:
                    predict_dones = model_wrapper.predict_done(batch_state.episodes, batch_state.object_infos)
                    batch_state.predict_dones = [
                        old_done or new_done
                        for old_done, new_done in zip(batch_state.predict_dones, predict_dones)
                    ]
                
                batch_state.update_metric()
                
                if args.disable_external_assistant or args.use_internalized_assistant:
                    assist_notices = None
                else:
